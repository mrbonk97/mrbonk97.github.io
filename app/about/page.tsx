      ...
      <main>
        <header className="pt-20 p-4 min-h-screen mx-auto max-w-4xl">
          ...
          <MotionP className="mt-16 font-medium opacity-80 text-center break-keep">
            ...
            현재는 금융 IT 분야에서 프론트엔드 개발과 보안 컨설팅 업무를
            수행하고 있습니다.
          </MotionP>
        </header>

        <section className="border-t">
          <MotionH4 className="py-16 text-rose-400 text-center text-7xl md:text-9xl font-bold">
            PROJECT
          </MotionH4>
          <div className="grid grid-cols-6 border-t">
            <div className="hidden md:block col-span-1 p-4 border-r text-right">
              <span className="sticky top-24 font-medium text-rose-400">
                개발 프로젝트
              </span>
            </div>
            ...
            <ul className="col-span-6 md:col-span-5 grid grid-cols-1 md:grid-cols-2 gap-4 p-4 mx-auto max-w-4xl w-full">
              {PROGRAMMING_PROJECT.map((item, idx) => {
                return (
                  <MotionLi
                    key={item.title}
                    className="md:aspect-video group"
                    delay={idx % 2 == 0 ? 0 : 0.25}
                  >
                    <Link
                      href={`/about/${item.id}`}
                    ...
                    >
                      <hgroup>
                        <h4 className="text-xl font-semibold opacity-80">
                          {item.title}
                        </h4>
              ...
              {SECURITY_PROJECT.map((item, idx) => (
                <MotionLi key={item.title} className="py-4 px-8 bg-secondary">
                  <div className="flex justify-between gap-4">
                    <h4 className="text-base md:text-xl font-semibold opacity-80">
                      {item.title}
                    </h4>
